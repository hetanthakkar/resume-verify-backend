...
class ProfileMatcher:
    def __init__(self, openai_key: str, batch_size: int = 3):
    ...
    async def _extract_experience_single(self, text: str) -> Dict:
        ...
        try:
            response = await self._make_openai_call(prompt, text)
            result = json.loads(response)
    ...
    async def _extract_experience_single(self, text: str, cache_key: str) -> Dict:
        """Extract experience details from a single text chunk."""
        if cache_key in self._text_cache:
            return self._text_cache[cache_key]
    ...
    def extract_experience_details(self, text: str) -> List[Dict]:
        ...
        response = openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a precise parser that extracts structured data and returns only valid JSON.",
                },
